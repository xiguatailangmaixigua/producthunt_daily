...
def run_script(script_path, description):
    ...
    try:
        ...
        logging.error(f"{description}执行失败: {e}")
        if e.stdout:
...
def main():
    ...
    scripts = [
        ...
        {
            "path": os.path.join(script_dir, "generate_wechat_html.py"),
            "description": "生成微信公众号HTML文件"
        }
    ]
    ...
    try:
        ...
        if choice == "1":
            ...
            run_script(draft_script, "保存到微信公众号草稿箱")
        elif choice == "2":
            # 直接发布到微信公众号
            wechat_script = os.path.join(script_dir, "publish_to_wechat.py")
            run_script(wechat_script, "发布到微信公众号")
        elif choice == "3":
            # 提示用户手动发布
            today = datetime.now().strftime('%Y-%m-%d')
            html_file_path = os.path.abspath(f"data/{today}_wechat.html")
            cover_image_path = os.path.abspath("assets/cover.jpg")
